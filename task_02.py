...
def user_move(count, limit, user):
    if user == 1:
        a = int(input(f'{appeals_user[random.randint(0, len(appeals_user) - 1)]}'))
    elif user == 2:
        a = int(input(f'{first_user} {appeals_user[random.randint(0, len(appeals_user) - 1)]}'))
    elif user == 3:
        a = int(input(f'{second_user} {appeals_user[random.randint(0, len(appeals_user) - 1)]}'))
        ...
        if my_choice2 == 2:
            ...
            while number_of_candies > limit_candies:
                if draw == 0:
                    ...
                    number_of_candies = bot_move(number_of_candies, limit_candies, my_choice3)
                if number_of_candies <= limit_candies:
                    break
                number_moves += 1
                print(f'Осталось конфет: {number_of_candies}')
